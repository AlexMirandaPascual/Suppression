...
def pandasMDAVwMSE1D(path_of_data: str, column_name: str, k: int, Change_value_for_the_centroid=True):
...
def find_error_pandasMDAVwMSE(path_of_data: str, column_name: str, k: int, ):
    df_list=pandasMDAVwMSE1D(path_of_data, column_name, k, Change_value_for_the_centroid=False)
    for i in range(len(df_list)):
        df=df_list[i]
        mean=df.mean()
...
def pandasMDAVwMSE2D(df2D, column_name_1: str, column_name_2: str, k: int, Change_value_for_the_centroid=True):
    df=df2D
    # Normalize the data
    ...
    principal=pd.concat([principal1, principal2], axis=1)
    df_list=[]
    
    while principal.shape[0]>=2*k:   
        centroide1=principal[column_name_1].mean()
        centroide2=principal[column_name_2].mean()
        # xr
        xr_df=(principal[column_name_1]-centroide1)**2 + (principal[column_name_2]-centroide2)**2
        xr=xr_df.max()
        xr_list=(principal[column_name_1]-xr)**2 + (principal[column_name_2]-xr)**2
        list_near_xr=xr_list.sort_values()
        xr_k_near=list_near_xr.iloc[0:k]
        index_xr=xr_k_near.index
        xr_cluster=principal.loc[index_xr]
        ...
        #xs
        xs=list_near_xr.iloc[-1]
        xs_list= (principal[column_name_1]-xs)**2 + (principal[column_name_2]-xs)**2
        list_near_xs=xs_list.sort_values()
        xs_k_near=list_near_xs.iloc[0:k]
        index_xs=xs_k_near.index
        xs_cluster=principal.loc[index_xs]
    ...
    if principal.shape[0]<k:
      ...
      #Calculate the mean of each cluster and create a list of mean values for each cluster
       ...
       for i in range(principal.shape[0]):
            ...
            mean_prov=(mean_df[column_name_1]-principal[column_name_1].iloc[i])**2+(mean_df[column_name_2]-principal[column_name_2].iloc[i])**2
            #index of the min distance
            # print(mean_prov[:])
            index=mean_prov.idxmin()
            # print(index)
            df_prov=df_list[index]
            one_register=pd.DataFrame(principal.loc[index_principal[i]])
            one_register=one_register.T
            df_prov=pd.concat([df_prov,one_register])
            df_list[index]=df_prov
