...
def generate3DmetricAverages2D(plot_path_start="Plots\\Age_eps=1_delta=SQR", path="File_graphic\\Age_eps=1_delta=SQR_combined.csv", epsilon=1, EpsDeltaChange=False, gaussian=False, smallsuppression=False, relative_error=True):
    df_all = pd.read_csv(path)
    Points=[]

    if relative_error==True:
        average = df_all["original_average"]
    else:
    ...
    if EpsDeltaChange==False:
        if gaussian==False:
            ...
            x=m
            y=M
            z=df["difference_laplace_M_minus_MoS"+file_name_realmean]
            file_name_middle="_difference_laplace_M_minus_MoS"+file_name_realmean
            ...
            x=m
            y=M
            z=df["difference_gaussian_M_minus_MoS"+file_name_realmean]
            file_name_middle = "_difference_gaussian_M_minus_MoS"+file_name_realmean
            for i in range(df.shape[0]):
                Points.append([x.iloc[i],y.iloc[i],z.iloc[i]])
    else:
        if gaussian==False:
            ...
            x=m
            y=M
            z=df["difference_laplace_MChangeEpsDelta_minus_MoS"+file_name_realmean]
            file_name_middle = "_difference_laplace_MChangeEpsDelta_minus_MoS"+file_name_realmean
            for i in range(df.shape[0]):
                Points.append([x.iloc[i],y.iloc[i],z.iloc[i]])
        else:
            ...
            x=m
            y=M
            z=df_gaussian["difference_gaussian_MChangeEpsDelta_minus_MoS"+file_name_realmean]
            file_name_middle = "_difference_gaussian_MChangeEpsDelta_minus_MoS"+file_name_realmean
