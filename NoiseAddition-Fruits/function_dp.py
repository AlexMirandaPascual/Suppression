    ...
    for i in range(len(m)):
        ...
        for j in range(longitud):
            if m[longitud-j-1]>=m[i]:
                ...
                generate_probabilities_csv(m=m[i],M=m[longitud-j-1], path_distances=path_average_distances, path_probabilidades=name_file)
            else:
                break
...
or not each element, probabilitic and dataset must be dataframe"""
...
def agregateLaplaceandGaussian(file="Apple_onlysupression.csv", upper_weight=140, lower_weight=60, upper_volume=105, lower_volume=45, epsilon=1, sensitivity=1, delta=None, real_weight=100, real_volume=80):
    carpet="File_graphic\\"
    ...
    for i in range(totalelement[0]):
        ...
        #difference L2 with weight and volume Laplacian
...
def agregateLaplaceandGaussianPrima(file="Apple_onlysupression.csv", upper_weight=140, lower_weight=60, upper_volume=105, lower_volume=45, epsilon=1, sensitivity=1, delta=None, real_weight=100, real_volume=80):
    carpet="File_graphic\\"
    df=pd.read_csv(carpet + file)
    totalelement=df.shape
    average_laplacian_weight_list=[]
    ...
    for i in range(totalelement[0]):
        ...
        sumtotal_volume=new_df["total_sum volume"]
        total_element=new_df["total_element"]
        delta_prima=calculate_delta_prima(delta=delta , m=new_df["m"])
...
def calculateAverageofelement(file="File_graphic\\Apple_onlysupression.csv", File_name="File_graphic\\AverageApple.csv"):
    ...
    df=pd.read_csv(file)
    header=["m", "M", "average weight", "average_laplacian weight","average_gaussian weight", "average volume", "average_laplacian volume","average_gaussian volume", "average_L2_laplacian_supression", "average_L2_gaussian_supression"]
    element=[[0]*10]
    myM=generatem_M()
...
def agregatefileoriginalPrima(path="fruits.csv", name_of_newfile="original", fruit_name="Apple", epsilon=1, delta=None, upper_weight=140, lower_weight=60, upper_volume=105, lower_volume=45, sensitivity=1, real_weight=100, real_volume=80 ):
    ...
    if delta==None:
        ...
        delta=delta
    header=["m", "M", "delta prima", "epsilon prima", "average weight", "average_laplacian weight", "average_gaussian weight", "average volume", "average_laplacian volume", "average_gaussian volume", "L2_laplacian_prima", "L2_gaussian_prima"]
    element=[[0]*12]
    myM=generatem_M()
    for i in range(len(myM)):
        # print(myM[i])
        ...
        average_gaussian_volume=sumtotal_gaussian_volume/total_element_gaussian_volume
         #difference L2 with weight and volume Laplacian
        L2_laplacian_prima=np.linalg.norm([average_laplacian_weight-real_weight,average_laplacian_volume-real_volume])
        #difference L2 with weight and volume Gaussian
        L2_gaussian_prima=np.linalg.norm([average_gaussian_weight-real_weight,average_gaussian_volume-real_volume]) 
        
        element.append([m, M, delta_prima, epsilon_prima, average_weight, average_laplacian_weight, average_gaussian_weight, average_volume, average_laplacian_volume, average_gaussian_volume, L2_laplacian_prima, L2_gaussian_prima])
    new_df=pd.DataFrame(element, columns=header)
...
def combining_averages(path_average_supression="File_graphic\\AverageApple.csv", path_average_prima="File_graphic\\originalApple prima.csv",
    ...
    for i in range(len(myM)):
        ...
        metric_laplacian=ave_prima["L2_laplacian_prima"]-ave_supre["average_L2_laplacian_supression"]
        metric_gaussian=ave_prima["L2_gaussian_prima"]-ave_supre["average_L2_gaussian_supression"]
